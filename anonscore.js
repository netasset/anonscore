...
const WALLET_REVIEWS = [{
  ...
  category: "desktop",
  os: "macOS · Windows · Linux",
  pitch: "Privacy-first desktop Bitcoin wallet with built-in WabiSabi CoinJoin and automatic coin labelling. Beginner-friendlier than Sparrow.",
...
function runEngine(utxos = [], txs = [], txCount = 0) {
  ...
  if (failed.find(f => f.key === "cj")) recs.push({
    ...
    detail: "The tools below each implement CoinJoin differently — pick what fits your setup and technical comfort level. Aim for anonymity sets of 50+ participants and time it during low-fee periods.",
    impact: 18,
    effort: "Medium",
    tools: [{
      name: "Wasabi Wallet",
      note: "Automatic WabiSabi CoinJoin, desktop, beginner-friendly"
    }, {
      name: "Sparrow Wallet",
      note: "Manual Whirlpool-compatible CoinJoin, desktop"
    }, {
      name: "Joinmarket",
      note: "Peer-to-peer maker/taker model, earn fees as a maker"
    }, {
      name: "JoinStr",
      note: "Nostr-coordinated CoinJoin, experimental"
    }, {
      name: "BTCPay Server",
      note: "Self-hosted, Payjoin + CoinJoin for merchants"
    }],
    key: "cj"
  });
...
function runLightningEngine(node = {}, channels = []) {
  ...
  if (chCount === 0) add("channels", "Channel Diversity", "warn", "No open channels found. A node with no channels has limited routing privacy and cannot receive spontaneous payments.", "medium", -6);else if (chCount < 3) add("channels", "Channel Diversity", "warn", `Only ${chCount} channel${chCount > 1 ? "s" : ""}. Low channel count limits routing path diversity, making payment flows easier to trace.`, "medium", -4);else add("channels", "Channel Diversity", "pass", `${chCount} channels — good routing diversity makes payment flows harder to trace.`, "clean", 0);
  const totalCap = channels.reduce((s, c) => s + (c.capacity || 0), 0);
  const maxCap = channels.length ? Math.max(...channels.map(c => c.capacity || 0)) : 0;
  if (totalCap > 0 && maxCap / totalCap > 0.8) add("cap_conc", "Channel Capacity Concentration", "warn", "Over 80% of your node capacity is in one channel. This makes your routing patterns highly predictable.", "medium", -6);else add("cap_conc", "Channel Capacity Concentration", "pass", "Your capacity is distributed across channels — good for routing privacy.", "clean", 0);
  ...
  if (openedChannels > 10) add("onchain", "On-Chain Channel Footprint", "warn", `${openedChannels} channel opens/closes are recorded on-chain. Each one is a permanent trace linking your Lightning and on-chain identity. Funding channels directly from KYC exchange withdrawals is the riskiest pattern.`, "medium", -5);else if (openedChannels > 0) add("onchain", "On-Chain Channel Footprint", "pass", `${openedChannels} channel${openedChannels > 1 ? "s" : ""} — modest on-chain footprint. Ensure you fund channels from non-KYC UTXOs to avoid linking your identities.`, "clean", 0);else add("onchain", "On-Chain Channel Footprint", "pass", "No open channels found — no on-chain channel footprint to analyse.", "clean", 0);
  score = Math.max(0, Math.min(100, Math.round(score)));
  const recs = [];
  const failed = checks.filter(c => c.status !== "pass");
...
function Dashboard({
  ...
  const issueCount = checks.filter(c => c.status !== "pass").length;
  const TABS = isMobile ? ["Fix It", "Overview", "UTXOs", "Transactions"] : ["Fix It", "Overview", "UTXOs", "Transactions", "Methodology"];
  useEffect(() => {
    if (!autoShare) return;
    const t = setTimeout(() => {
  ...
  }, ["Fix It", "Overview", "UTXOs", "Transactions"].map(t => React.createElement("button", {
    key: t,
    onClick: () => setTab(t),
    style: {
      flex: 1,
  ...
  }, t === "Fix It" ? "★" : t === "Overview" ? "◎" : t === "Transactions" ? "↔" : "⬡"), t))), shareOpen && React.createElement(ShareCard, {
    score: score,
    grade: grade,
    checks: checks,
    address: address,
